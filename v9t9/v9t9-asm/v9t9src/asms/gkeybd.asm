...
; ********************************************
; porting to german keyboard layout
; by Reiner Ziegler 3/20/1996
; E-Mail: Compuserve 100137,3150
; ********************************************
...
;
;	t  Figure out mouse/joystick status, and set "isjoystick"
...
;	KEYBOARD:	Restart.
;	
;	t  Save the incoming LED state.
;	t  Clear the CRU bitmaps and the fast keyboard flags.
;===========================================================================
...
;	KEYBOARD:	Restop.
;	
;	t  Clear the keyboard buffers.
;	t  Update the BIOS keyboard LED flag.
;===========================================================================
...
kb02:
	cmp	al,0
	jne	kb03
	or	stateflag,ctrlbreakpressed	; buffer overflow!  panic!
	mov	happymessage,1
	jmp	kbend
...
setkeyboardmap	proc	near
	...
	push	cx
     	...
     	test    shift,s_alt             ; alt key ?   !!!! does not work !!!!
        jz      weiter
        cmp     curscan,10h             ; test Q (means @)
        jne     weiter
        mov     al,039h                 ; new CRU matrix code for @
        inc     altflag
        and     tishift,not s_alt
        and     shift,not s_alt         ; reset alt states
        jmp     kbd_next
...
kbd_change:
        inc     gerflag
        and     ah,not s_shift
        and     curscan,not s_shift
        and     tishift,not s_shift
        and     shift,not s_shift       ; reset shift states
...
skbmout:
        cmp     gerflag,1               ; shift modified ?
        jne     skb_alt
        dec     gerflag
        or      curscan,s_shift
        or      tishift,s_shift
        or      shift,s_shift           ; set like before
        jmp     skb_ok
skb_alt:cmp     altflag,1               ; alt modified ?
        jne     skb_ok
        dec     altflag
        or      shift,s_alt                
        or      tishift,s_alt           ; set like before
...
nkeys	db	255,15,'1234567890-=',8		; 15
	db	255,'qwertzuiop[]',13		; 14
	db	255,'asdfghjkl;',39		; 12
	db	'`',255,'\yxcvbnm,./',255	; 14
	db	'*',255,' ',255			; 4

skeys	db	255,15,'!@#$%^&*()_+',8
	db	255,'QWERTZUIOP{}',13
	db	255,'ASDFGHJKL:"'
	db	'~',255,'|YXCVBNM<>?',255
	db	'*',255,' ',255
