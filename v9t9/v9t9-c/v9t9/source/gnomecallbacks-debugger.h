...
#define DBG_BG_WRITTEN(s)	(&(s)->bg[GTK_STATE_SELECTED])
                                        ...
                                        gpointer         user_data)
{
	...
	width = gdk_string_width(gtk_style_get_font(s), "F");
...
//	width = debugger_hex_dump_chars_to_bytes(allocation->width / (gdk_string_width(s->font, "F")) - 1);
	...
	width = debugger_hex_dump_chars_to_bytes((allocation->width) / (gdk_string_width(gtk_style_get_font(s), "F")));
	height = (allocation->height - 4) / gdk_string_height(gtk_style_get_font(s), "!");
...
setup_debugger_instruction_box(void)
{
	...
	width = gdk_string_width(gtk_style_get_font(s), "^") * 40;
	height = gdk_string_height(gtk_style_get_font(s), "!") * 16;
//	gtk_style_unref(s);
