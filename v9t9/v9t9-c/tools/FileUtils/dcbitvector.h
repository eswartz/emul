/*
 *	This module implements resizable bitvectors with ranges, used in cases
 *	where many bitvectors of a certain class are allocated but known to
 *	be sparse.  Rather than resort to "fast" sets and risk wasting 64x
 *	more memory than necessary in the worst case, we sacrifice a small amount
 *	of run time to keep only the live bits in a bitvector in memory.
 */
