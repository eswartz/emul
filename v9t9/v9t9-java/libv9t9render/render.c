...
#define COPY(n,s)	do { \
		dest[n*3] = src[s*3];	dest[n*3+1] = src[s*3+1];	dest[n*3+2] = src[s*3+2]; } while(0)

#define BLEND(n, s)	do { \
		dest[n*3] = (src[s*3+0]+src[s*3+3])>>1;	\
		dest[n*3+1] = (src[s*3+1]+src[s*3+4])>>1 ;\
		dest[n*3+2] = (src[s*3+2]+src[s*3+5])>>1; \
		} while(0)

#define BLENDL(n, s)	do { \
		dest[n*3] = (src[s*3+0]*3+src[s*3+3])>>2;	\
		dest[n*3+1] = (src[s*3+1]*3+src[s*3+4])>>2;\
		dest[n*3+2] = (src[s*3+2]*3+src[s*3+5])>>2; \
		} while(0)

#define BLENDR(n, s)	do { \
		dest[n*3] = (src[s*3+0]+src[s*3+3]*3)>>2;	\
		dest[n*3+1] = (src[s*3+1]+src[s*3+4]*3)>>2;\
		dest[n*3+2] = (src[s*3+2]+src[s*3+5]*3)>>2; \
		} while(0)
...
static void iterRow_2(unsigned char *destptr, const unsigned char* srcptr, int destrowstride, int rowstride,
	...
	while (cnt--) {
		(*scaleRow)(destptr, srcptr, width);
		destptr += destrowstride;
		memcpy(destptr, destptr - destrowstride, destWidth * 3);
		destptr += destrowstride;
		srcptr += rowstride;
	}
}
...
static void iterRow_7_2(unsigned char *destptr, const unsigned char* srcptr, int destrowstride, int rowstride,
	...
	while (cnt >= 2) {
		...
		(*scaleRow)(destptr, srcptr, width);
		memcpy(destptr + destrowstride, destptr, destWidth * 3);
		// draw fifth/sixth row
		(*scaleRow)(destptr + destrowstride * 5, srcptr + rowstride, width);
		memcpy(destptr + destrowstride * 6, destptr + destrowstride * 5, destWidth * 3);
		// blend them
