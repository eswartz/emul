...
public class LPCSpeech {
	...
	private int muldiv(int i, int j) {
		return ((i * j) >> 15);
	}
...
//				params.kVal[3] = getNumber(RomTables.k4table[params.kParam[3]]);
		...
		Logging.writeLogLine(0, settings.get(ISpeechChip.settingLogSpeech), "Equation: " + params);

		Logging.writeLogLine(4, settings.get(ISpeechChip.settingLogSpeech), "energy: " + oldParams.energyParam + " => "
				+ params.energyParam + ", pitch: " + oldParams.pitchParam + " => " + params.pitchParam);
	}
	...
	/**
	 * One LPC frame consists of calculating a speech waveform and outputting it for
	 * a set of parameters.
	 * 
