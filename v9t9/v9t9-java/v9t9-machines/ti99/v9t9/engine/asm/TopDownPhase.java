...
import v9t9.engine.asm.DataWordListOperand;
...
import v9t9.engine.asm.LabelOperand;
import v9t9.engine.asm.Routine;
import v9t9.engine.asm.RoutineOperand;
import v9t9.engine.asm.UnknownRoutine;
...
public class TopDownPhase extends Phase {
	 ...
	 * @return
	 */
	private Operand handleLabel(IHighLevelInstruction inst, MachineOperand mop) {
	 ...
	 */
	private Operand handleLabel(IHighLevelInstruction inst, int addr) {
	...
	Label[] findJumpTable(IHighLevelInstruction caller,
		...
		while (inst != null && maxInsts-- > 0) {
			...
			if (inst.getInst().getOp2() instanceof MachineOperand &&
					((MachineOperand) inst.getInst().getOp2()).isRegisterReference(reg)) {
				MachineOperand mop2 = ((MachineOperand) inst.getInst().getOp2());
				if (mop2.isRegister()) {
					// directly set
					if (inst.getInst().getOp1() instanceof MachineOperand) {
	...
	private int guessJumpTableSize(IHighLevelInstruction inst, MachineOperand9900 fromOp1) {
		...
		while (inst.getPrev() != null) {
			...
			if (inst.getInst().getOp1() instanceof MachineOperand
					&& ((MachineOperand) inst.getInst().getOp1()).isRegister(reg)) {
				if (inst.getInst().getOp2() instanceof MachineOperand == false) {
