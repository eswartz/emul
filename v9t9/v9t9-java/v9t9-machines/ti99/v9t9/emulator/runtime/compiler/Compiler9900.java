...
import org.ejs.coffee.core.launch.CommandLauncher;
import org.ejs.coffee.core.launch.ProcessClosure;
...
public class Compiler9900 extends CompilerBase {
	...
	private void updateStatus(int handler, CompileInfo info) {
		...
		switch (handler) {
		case Status.stset_NONE:
			return;
		case Status.stset_ALL:
			// just a note that Status should be up to date, for future work
			return;
		case Status.stset_INT:
    ...
    public void generateInstruction(int pc, RawInstruction rawins,
        ...
        if (ins.getInfo().stsetBefore != Status.stset_NONE) {
        ...
        if (ins.getInfo().stsetAfter != Status.stset_NONE) {
    ...
    private void flushOperands(Instruction9900 ins, CompileInfo info) {
        ...
        if ((ins.getInfo().writes & v9t9.engine.cpu.InstInfo.INST_RSRC_CTX) != 0) {
            ...
            ilist.append(new ALOAD(info.localStatus));
            ilist.append(info.ifact.createInvoke(v9t9.emulator.runtime.cpu.CpuState.class.getName(),
	...
	private CompileInfo setupCompileInfo(InstructionFactory ifact,
        ...
        info.cpuIndex = pgen.addFieldref(v9t9.emulator.runtime.compiler.CompiledCode.class.getName(), // className,
                "cpu", Utility.getSignature(v9t9.emulator.runtime.cpu.Cpu.class.getName()));
        ...
        info.vdpIndex = pgen.addFieldref(v9t9.emulator.runtime.compiler.CompiledCode9900.class.getName(), // className,
                "vdpMmio", Utility.getSignature(v9t9.emulator.hardware.memory.mmio.VdpMmio.class.getName()));
        info.gplIndex = pgen.addFieldref(v9t9.emulator.runtime.compiler.CompiledCode9900.class.getName(), // className,
        ...
        info.breakList.append(info.ifact.createInvoke(v9t9.emulator.runtime.cpu.Cpu.class.getName(),
	...
	private void compileLoadCpuState(InstructionFactory ifact,
	    ...
	    ilist.append(ifact.createInvoke(v9t9.emulator.runtime.cpu.CpuState.class.getName(),
	...
	private InstructionHandle compileStoreCpuState(
	    ...
	    ilist.append(InstructionConstants.DUP);
	    ilist.append(new ILOAD(info.localPc));
	    ilist.append(ifact.createInvoke(v9t9.emulator.runtime.cpu.CpuState.class.getName(), "setPC",
