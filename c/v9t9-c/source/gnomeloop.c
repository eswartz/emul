...
#if defined(GTK_VIDEO) && defined(GTK_KEYBOARD)
...
//	if (!(GTK_OBJECT(v9t9_command_log)->flags & GTK_DESTROYED))//2.0
	if (v9t9_command_log) 
	{
		gtk_text_buffer_insert_at_cursor(gtk_text_view_get_buffer(GTK_TEXT_VIEW(v9t9_command_log)), 
																  //font, 
//						color, 
//						NULL, 
						text, 
...
#endif	// defined(GTK_VIDEO) && defined(GTK_KEYBOARD)
