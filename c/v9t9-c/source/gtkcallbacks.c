/*
  gtkcallbacks.c				-- GTK callback definitions
...
#include "gtkcallbacks-v9t9.h"
#include "gtkcallbacks-command.h"
#include "gtkcallbacks-modules.h"
#include "gtkcallbacks-disks.h"
#include "gtkcallbacks-debugger.h"
#include "gtkcallbacks-memory.h"
	...
	if (enabled) {
		if (!VALID_WINDOW(debugger_window)) {
...

			{
				...
				const char *fontname = "-monotype-courier new-semilight-r-normal-*-*-120-*-*-m-*-*-*";
				GdkFont *font = gdk_font_load(fontname);
				if (font) 
				{
					rcstyle->font = font;
					gtk_widget_set_style(debugger_window, rcstyle);	
				}
			}
...
create_rom_file_selection (void)
{
  ...
  gtk_object_set_data (GTK_OBJECT (rom_file_selection), "rom_file_selection", rom_file_selection);
  ...
  gtk_object_set_data (GTK_OBJECT (rom_file_selection), "ok_button2", ok_button2);
  gtk_widget_show (ok_button2);
  GTK_WIDGET_SET_FLAGS (ok_button2, GTK_CAN_DEFAULT);

  cancel_button2 = GTK_FILE_SELECTION (rom_file_selection)->cancel_button;
  gtk_object_set_data (GTK_OBJECT (rom_file_selection), "cancel_button2", cancel_button2);
