 ...
 *
 * modified (c) 1999-2001 Ed Swartz for v9t9
 *
...
//#if GTK_MAJOR_VERSION*10+GTK_MINOR_VERSION >= 13
//#include "gtk/fnmatch.h"
//#else
//#define g_filename_from_utf8(str,v)	g_strdup(str)
//#define g_filename_to_utf8(str,v)	g_strdup(str)
#include "fnmatch.h"
//#endif

#ifndef MAXPATHLEN
#define MAXPATHLEN OS_MAXPATHLEN
#endif
...
static void gtk_file_selection_class_init    (V99FileSelectionClass *klass);
static void gtk_file_selection_init          (V99FileSelection      *filesel);
...
static void gtk_file_selection_populate      (V99FileSelection      *fs,
					      gchar                 *rel_path,
					      gint                   try_complete);
static void gtk_file_selection_abort         (V99FileSelection      *fs);

static void gtk_file_selection_update_history_menu (V99FileSelection       *fs,
						    gchar                  *current_dir);
 ...
 */
int
translate_win32_path(V99FileSelection *filesel)
{
  int updated = 0;
  gchar *path;
...
GtkType
v99_file_selection_get_type (void)
{
  ...
  if (!file_selection_type)
    {
      ...
      {
	"V99FileSelection",
	sizeof (V99FileSelection),
	sizeof (V99FileSelectionClass),
...
static void
gtk_file_selection_class_init (V99FileSelectionClass *class)
{
...
static void
gtk_file_selection_init (V99FileSelection *filesel)
{
...
//  GtkWidget *scrolled_win;
  ...
  v99_file_selection_show_fileop_buttons(filesel);
  ...
  filesel->list_hbox = gtk_hbox_new (FALSE, 6);
  gtk_box_pack_start (GTK_BOX (filesel->main_vbox), filesel->list_hbox, TRUE, TRUE, 0);
  gtk_widget_show (filesel->list_hbox);
  ...
  filesel->dir_list_scrolled_win = gtk_scrolled_window_new (NULL, NULL);
  gtk_container_add (GTK_CONTAINER (filesel->dir_list_scrolled_win), filesel->dir_list);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (filesel->dir_list_scrolled_win),
				  GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  gtk_container_set_border_width (GTK_CONTAINER (filesel->dir_list_scrolled_win), 5);
  gtk_box_pack_start (GTK_BOX (filesel->list_hbox), filesel->dir_list_scrolled_win, TRUE, TRUE, 0);
  gtk_widget_show (filesel->dir_list);
  gtk_widget_show (filesel->dir_list_scrolled_win);
...
//  gtk_widget_set_usize (filesel->file_list, FILE_LIST_WIDTH, FILE_LIST_HEIGHT);
  ...
  v99_file_selection_set_file_list_columns(filesel, 1, (gchar **)file_title, NULL, NULL);
  v99_file_selection_set_file_list_active(filesel, TRUE);
  gtk_widget_set_usize (filesel->file_list, FILE_LIST_WIDTH, FILE_LIST_HEIGHT);
...
GtkWidget*
v99_file_selection_new (const gchar *title)
{
  V99FileSelection *filesel;

  filesel = gtk_type_new (V99_TYPE_FILE_SELECTION);
...
void
v99_file_selection_show_fileop_buttons (V99FileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (V99_IS_FILE_SELECTION (filesel));
...
void       
v99_file_selection_hide_fileop_buttons (V99FileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (V99_IS_FILE_SELECTION (filesel));
...
void
v99_file_selection_set_filename (V99FileSelection *filesel,
...
gchar*
v99_file_selection_get_filename (V99FileSelection *filesel)
{
  ...
  g_return_val_if_fail (V99_IS_FILE_SELECTION (filesel), nothing);
 ...
 text = gtk_entry_get_text (GTK_ENTRY (filesel->selection_entry));
  if (text)
    {
		filename = g_filename_from_utf8 (cmpl_completion_fullname (text, filesel->cmpl_state), -1, 0, 0, 0);
...
void
v99_file_selection_complete (V99FileSelection *filesel,
			     const gchar      *pattern)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (V99_IS_FILE_SELECTION (filesel));
...
gtk_file_selection_destroy (GtkObject *object)
{
  ...
  g_return_if_fail (V99_IS_FILE_SELECTION (object));

  filesel = V99_FILE_SELECTION (object);
...
void
v99_file_selection_set_file_list_columns(V99FileSelection *filesel,
										 int columns,
										 gchar *titles[],
										 ...
										 v99_file_selection_file_get ghandler)
{
	...
	if (filesel->file_list) {
		gtk_widget_destroy(filesel->file_list);
		filesel->file_list_select_row_handler_id = 0L;
	}

	filesel->file_list = gtk_clist_new_with_titles(columns, titles);
	filesel->user_file_append = ahandler;
	filesel->user_file_get = ghandler;

	gtk_widget_set_usize (filesel->file_list, FILE_LIST_WIDTH * columns / 2, FILE_LIST_HEIGHT);
	for (i = 0; i < columns; i++) {
		gtk_clist_set_column_width(GTK_CLIST(filesel->file_list), i, FILE_LIST_WIDTH / 2);
	}

	gtk_clist_column_titles_active (GTK_CLIST (filesel->file_list));

	if (filesel->file_list_scrolled_win) {
		gtk_widget_destroy(filesel->file_list_scrolled_win);
	}
	filesel->file_list_scrolled_win = gtk_scrolled_window_new (NULL, NULL);
	gtk_container_add (GTK_CONTAINER (filesel->file_list_scrolled_win), filesel->file_list);
	gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (filesel->file_list_scrolled_win),
									GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
	gtk_container_set_border_width (GTK_CONTAINER (filesel->file_list_scrolled_win), 5);
	gtk_box_pack_start (GTK_BOX (filesel->list_hbox), 
						filesel->file_list_scrolled_win, TRUE, TRUE, 0);

  gtk_widget_show (filesel->file_list);
  gtk_widget_show (filesel->file_list_scrolled_win);
}

void		
v99_file_selection_set_file_list_active(V99FileSelection *filesel, gboolean active)
{
	if (active) {
		if (filesel->file_list_select_row_handler_id) {
			return;
		}
		filesel->file_list_select_row_handler_id = 
		...
		gtk_signal_disconnect(GTK_OBJECT (filesel->file_list), 
							  filesel->file_list_select_row_handler_id);
		filesel->file_list_select_row_handler_id = 0;
	}
}

/*
 *	Add a List to a vbox on the left side of the dialog
 *	which contains a list of paths from 'path_list'.  
 */
										 ...
										 gchar *path_list)
{
	...
	GtkCList *clist;
	gchar *ptr;
	char tmp[OS_PATHSIZE], *tmplist[2];

	if (!filesel->path_list_vbox) {
		vbox = gtk_vbox_new(FALSE, 4);
		filesel->path_list_vbox = vbox;
		gtk_widget_ref(vbox);
		gtk_object_set_data_full (GTK_OBJECT (vbox), 
								  "path_list_vbox", vbox,
								  (GtkDestroyNotify) gtk_widget_unref);
		gtk_box_pack_start (GTK_BOX (filesel->list_hbox), vbox, TRUE, TRUE, 0);
		gtk_widget_show (vbox);
	} else {
		vbox = filesel->path_list_vbox;
	}
	...
	clist = GTK_CLIST(w);
	gtk_clist_set_column_width (clist, 0, 64);
	gtk_clist_set_selection_mode (clist, GTK_SELECTION_SINGLE);

	gtk_clist_set_column_title(clist, 0, path_list_name);
	gtk_clist_column_titles_show (clist);
	  gtk_widget_set_usize (GTK_WIDGET(clist), FILE_LIST_WIDTH, FILE_LIST_HEIGHT);
	...
	while (path_list && *path_list) {
		ptr = strchr(path_list, OS_ENVSEP);
		if (!ptr)
			ptr = strpbrk(path_list, OS_ENVSEPLIST);
		if (!ptr)
			ptr = path_list + strlen(path_list);

		strcpyn(tmp, path_list, ptr - path_list, OS_PATHSIZE);
		gtk_clist_append(clist, tmplist);
		path_list = (*ptr) ? ptr + 1 : NULL;
	}

	/* now widen the list and show it */
...
static void
gtk_file_selection_fileop_error (V99FileSelection *fs, gchar *error_message)
{
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *button;
...
{
  V99FileSelection *fs = data;

  g_return_if_fail (fs != NULL);
  g_return_if_fail (V99_IS_FILE_SELECTION (fs));
...
gtk_file_selection_create_dir_confirmed (GtkWidget *widget, gpointer data)
{
  ...
  sys_full_path = g_filename_from_utf8 (full_path, -1, 0, 0, 0);
...
gtk_file_selection_rename_file_confirmed (GtkWidget *widget, gpointer data)
{
  ...
  sys_new_filename = g_filename_from_utf8 (new_filename, -1, 0, 0, 0);
  sys_old_filename = g_filename_from_utf8 (old_filename, -1, 0, 0, 0);
			      ...
			      gpointer     user_data)
{
  ...
  if (event->keyval == GDK_Tab)
    {
      fs = V99_FILE_SELECTION (user_data);
#ifdef G_WITH_CYGWIN
      translate_win32_path(fs);
#endif
...
static void 
gtk_file_selection_update_history_menu (V99FileSelection *fs,
			       ...
			       gpointer user_data)
{
  ...
  if (fs->user_file_get) {
  	  fs->user_file_get(fs, GTK_CLIST(fs->file_list), row, &temp);
  } else {
...
static void
gtk_file_selection_populate (V99FileSelection *fs,
			     gchar            *rel_path,
			     gint              try_complete)
{
  CompletionState *cmpl_state;
  ...
  dir_list_width = gdk_string_width(gtk_style_get_font(fs->dir_list->style),".." G_DIR_SEPARATOR_S);
  ...
  while (poss)
    {
	  ...
	  text[0] = filename;
		...
		{
			int width = gdk_string_width(gtk_style_get_font(fs->dir_list->style),
					       filename);
		  row = gtk_clist_append (GTK_CLIST (fs->dir_list), text);
	    ...
	    {
		  if (fs->user_file_append) {
			  row = fs->user_file_append(fs, GTK_CLIST (fs->file_list), 
...
static void
gtk_file_selection_abort (V99FileSelection *fs)
{
  gchar err_buf[256];
...
cmpl_init_state (void)
{
  ...
  utf8_cwd = g_filename_to_utf8 (sys_getcwd_buf, -1, 0, 0, 0);
	     ...
	     CompletionState* cmpl_state)
{
  ...
  if (FALSE)
      ...
      else
	{
	  ...
	  gchar *utf8_curdir = g_filename_to_utf8 (sys_curdir, -1, 0, 0, 0);
  ...
  if(!cmp_len)
    {
      ...
      gchar *utf8_home = g_filename_to_utf8 (homedir, -1, 0, 0, 0);
      ...
      if (!pwd)
	{
      ...
      utf8_dir = g_filename_to_utf8 (pwd->pw_dir, -1, 0, 0, 0);
...
open_new_dir(gchar* dir_name, struct stat* sbuf, gboolean stat_subdirs)
{
  ...
  sys_dir_name = g_filename_from_utf8 (dir_name, -1, 0, 0, 0);
  ...
  for(i = 0; i < entry_count; i += 1)
      ...
      if(!dirent_ptr)
	{
      ...
      sent->entries[i].entry_name = g_filename_to_utf8 (dirent_ptr->d_name, -1, 0, 0,0);
...
correct_dir_fullname(CompletionDir* cmpl_dir)
{
  ...
  /* Ends with /.. (\..) ? */
      ...
      if(cmpl_dir->fullname + length - 3 == first_slash)
	{
      ...
      sys_filename = g_filename_from_utf8 (cmpl_dir->fullname, -1, 0, 0, 0);
...
find_parent_dir_fullname(gchar* dirname)
{
  ...
  sys_dirname = g_filename_from_utf8 (dirname, -1, 0, 0, 0);
  ...
  result = g_filename_to_utf8 (sys_cwd, -1, 0, 0, 0);
  g_free (sys_cwd);
...
get_pwdb(CompletionState* cmpl_state)
{
  ...
  while ((pwd_ptr = getpwent()) != NULL)
    {
		utf8 = g_filename_to_utf8 (pwd_ptr->pw_name, -1, 0, 0, 0);
      len += strlen (utf8);
      g_free (utf8);
      utf8 = g_filename_to_utf8 (pwd_ptr->pw_dir, -1, 0, 0, 0);
      len += strlen (utf8);
      g_free (utf8);
      len += 2;
      count += 1;
