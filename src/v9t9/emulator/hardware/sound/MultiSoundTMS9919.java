...
public class MultiSoundTMS9919 implements SoundProvider {
	...
	public MultiSoundTMS9919(Machine machine) {
		...
		for (int i = 0; i < 5; i++) {
			chips[i] = new SoundTMS9919(machine);
		}
		voices = null;
	}
	...
	public synchronized SoundVoice[] getSoundVoices() {
		if (voices == null) {
			...
			for (SoundTMS9919 chip : chips) {
				vcnt += chip.getSoundVoices().length;
			}
			voices = new SoundVoice[vcnt];
			int idx = 0;
			for (SoundTMS9919 chip : chips) {
				SoundVoice[] chipVoices = chip.getSoundVoices();
				System.arraycopy(chipVoices, 0, voices, idx, chipVoices.length);
				idx += chipVoices.length;
			}
		}
		return voices;
	}
	...
	public void setAudioGate(int addr, boolean b) {
		// assume it's the first one... Idon't think the audio gates are wired to the chips
	...
	public void setSoundHandler(SoundHandler soundHandler) {
		...
		for (SoundTMS9919 chip : chips) {
			chip.setSoundHandler(soundHandler);
		}
	}
