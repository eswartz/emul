...
import org.ejs.eulang.ast.IAstTypeOfExpr;
 ...
 *
 */
public class AstTypeOfExpr extends AstTypedExpr implements IAstTypeOfExpr {
	 ...
	 * @param type
	 */
	public AstTypeOfExpr(IAstTypedExpr expr) {
		setExpr(expr);
	}
	 ...
	 */
	@Override
	public IAstTypeOfExpr copy() {
		return fixup(this, new AstTypeOfExpr(doCopy(node)));
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		AstTypeOfExpr other = (AstTypeOfExpr) obj;
		if (node == null) {
			if (other.node != null)
				return false;
	...
	public boolean inferTypeFromChildren(TypeEngine typeEngine)
		...
		changed |= updateType(this, node.getType());
	...
	public void validateChildTypes(TypeEngine typeEngine) throws TypeException {
		if (node.getType() == null || !node.getType().isComplete()) {
			throw new TypeException(this, "cannot determine type of expression in typeof");
		}
	}
	...
	public boolean simplify(TypeEngine engine) {
		...
		if (node.getType() != null && node.getType().isComplete()) {
			// replace this node as an IAstType in the parent
			IAstType typeof = new AstType(engine.getRealType(node.getType()));
			typeof.setSourceRef(getSourceRef());
			getParent().replaceChild(this, typeof); 
			return true;
		}
		return changed;
	}
