...
public class DumpFullReporterMFP201 implements InstructionListener {
	...
	public void executed(InstructionWorkBlock before, InstructionWorkBlock after) {
		...
		dumpFullMid(before, (MachineOperandMFP201)before.inst.getOp1(), (MachineOperandMFP201)before.inst.getOp2(), dumpfull);
		dumpFullEnd(after, before.cycles, (MachineOperandMFP201)after.inst.getOp1(), (MachineOperandMFP201)after.inst.getOp2(), dumpfull);
	}
	...
	private void dumpFullMid(InstructionWorkBlock iinstructionWorkBlock,
			MachineOperandMFP201 mop1, MachineOperandMFP201 mop2,
	...
	private void dumpFullEnd(InstructionWorkBlock iinstructionWorkBlock, 
			...
			MachineOperandMFP201 mop2, PrintWriter dumpfull) {
		String str;
		if (mop1.type != MachineOperand.OP_NONE
		...
		dumpfull.print(  "sr="
		        + Integer.toHexString(cpu.getStatus().flatten() & 0xffff)
		                .toUpperCase() + " sp="
		        + Integer.toHexString(cpu.getStack() & 0xffff).toUpperCase());
