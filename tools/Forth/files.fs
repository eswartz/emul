...
\   02111-1307, USA.  
\
\	$Id: files.fs,v 1.17 2009-01-03 23:46:45 ejs Exp $
...
\	Behavior --
\
\	We can manage "binary" and traditional files.  The
\	p>bin? word tells if this is the case.  
...
\	Query the dirty flag (for binary files only)
\
: p>drt?		( pab -- )
	p>fl vc@ b<drt AND 0<>
;

\	Set the dirty flag (for binary files only)
\
: p>drt!		( pab -- )
	p>fl dup vc@ b<drt OR swap vc!
;

\	Reset the dirty flag (for binary files only)
\
: p>drt0		( pab -- )
...
Code <s+o					( sec offs -- doffs )
\	>r $100 um* 
\	r> s>d d+
	mov TOS , T1		\ T1 = low bytes, TOS = high bytes
	mov *SP , TOS		\ #secs
	mov TOS , T2
	srl TOS , 8 #		\ high bytes
	sla T2 , 8 #		\ low bytes added
	a T2 , T1
	jnc 1 $f
	inc TOS
1 $:
...
: OPEN-FILE		( caddr u fam -- fileid ior )
	...
	$0 and if
		...
		dup >r
		$8 rshift $ff and	( caddr u reclen )
		?dup 0= if &80 then
		newpab r> $80 xor  	( pab fam )
...
: pabread		( caddr ur fileid -- ur ior )
	>r
	$2 r@ vc!				\ READ op
	r@ 8 dsrlnk
	?dup 0= if
...
: READ-FILE		( caddr ur fileid -- ur ior )
	...
	dup p>bin? if
		...
		dup 6 = if
			...
			\ empty line -- fake a single blank on the line
			bl r@ c!
			2drop
			1 0
		then then
...
: INCLUDED ( addr c -- )
	r/o DIS VAR open-file ?dup if ." not found: " . cr 
	else
		dup >r include-file
		r> close-file drop
	then
