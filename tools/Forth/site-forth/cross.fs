...
\ modifications by Edward Swartz for building V9t9 FORTH
...
:noname ( c-addr u -- )
    ...
    dup c, here swap chars dup allot move ; IS string,
...
\ ejs
: erase ( taddr u -- )	over + swap do T 0 i c! loop H ;
...
: >tag cell+ ;		\ indecates type of reference: 0: call, 1: address, 2: doer; 3: by
: >taddr cell+ cell+ ;
: >ghost 3 cells + ;
: >file 4 cells + ;
: >line 5 cells + ;
...
\ abort
: is-resolved   ( ghost -- )
  >link @ T colon, H  ;	\ compile-call
[ELSE]
: is-resolved   ( ghost -- )
\  >link @ T a, H  ;	\ compile-call
  >link @
\ ." is-resolved at " there . ." with " dup .  cr
	T colon, H  ;	\ compile-call
[THEN]
...
: flag! ( 8b -- )   tlast @ dup >r T c@ or r> c! H ;        \ EJS: xor -> or
...
:noname  ( addr count -- )
dup T c, H bounds  ?DO  I c@ T c, H  LOOP ; 				IS string,
:noname  ( "name" -- )  bl word count T string, cfalign H ; IS name,
:noname  ( -- ) ( dummy ) ; 								IS view,
>CROSS
...
: .symtabentry
    >in @ cr ." sym:"  there 4 0.r [char] : emit bl word count .sym cr  >in !   \ EJS
;
...
: Alias    ( cfa -- ) \ name
	...
	.sourcepos ." needs prim: " >in @ bl word count type >in ! cr
    THEN
    (THeader over resolve T colon, H 80 flag! ;
[endif]
...
\ ejs:  [compile] is the ANS forth version
